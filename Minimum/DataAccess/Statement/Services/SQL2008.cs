...
{
    internal class SQL2008Statement : IStatement
    {
        ...
        {
            SkipCriteria offset = criterias.FirstOrDefault(c => c.Type == CriteriaType.Skip) as SkipCriteria;
            ...
            if (offset != null)
            {
                ...
                select.Append("* FROM (");
                select.Append("SELECT ");
                string order = SelectOrder(map, criterias);
                Property identity = map.Properties.FirstOrDefault(p => p.IsIdentity);
                select.Append("ROW_NUMBER() OVER (" + (order != null ? order : " ORDER BY " + (identity != null ? map.Alias + "." + identity.ColumnName : "1")) + " ) AS _RowNumber, ");
                ...
                select.Append(") AS RESULTS WHERE RESULTS._RowNumber > " + offset.Value.ToString());
            }
            else
            {
                select.Append("SELECT ");
